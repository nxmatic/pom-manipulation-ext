...
/**
 * Provides a convenient way of passing around related information about a Maven project without passing multiple
 * parameters. The model in this class represents the model that is being modified by the extension. Also stored is the
 * original POM file related to these models.
 *
 * @author jdcasey
 */
public class Project {
    public static final String MANIPULATED_POM_NAME = "pom-manipulation-ext.xml";
    ...
    /**
     * Model undergoing modification during execution. This model is what will eventually be written back to disk.
     */
    ...
    public int hashCode() {
        ...
        if (pom != null && !MANIPULATED_POM_NAME.equals(pom.getName())) {
    ...
    /**
     * This method will scan the dependencies in the potentially active Profiles in this project and return a fully
     * resolved list. Note that this will only return full dependencies not managed i.e. those with a group, artifact
     * and version. Note that while updating the {@link Dependency} reference returned will be reflected in the Model as
     * it is the same object, if you wish to remove or add items to the Model then you must use {@link #getModel()}
     *
    ...
    /**
     * This method will scan the dependencies in the potentially active Profiles in this project and return a fully
     * resolved list. Note that this will return all dependencies including managed i.e. those with a group, artifact
     * and potentially empty version. Note that while updating the {@link Dependency} reference returned will be
     ...
     * this project and return a fully resolved list. Note that while updating the {@link Dependency} reference returned
     * will be reflected in the Model as it is the same object, if you wish to remove or add items to the Model then you
    ...
    /**
     * This method will scan the plugins in this project and return a fully resolved list. Note that while updating the
     * {@link Plugin} reference returned will be reflected in the Model as it is the same object, if you wish to remove
    ...
    /**
     * This method will scan the plugins in the pluginManagement section of this project and return a fully resolved
     * list. Note that while updating the {@link Plugin} reference returned will be reflected in the Model as it is the
     * same object, if you wish to remove or add items to the Model then you must use {@link #getModel()}
     *
     * @param session MavenSessionHandler, used by {@link PropertyResolver}
    ...
    /**
     * This method will scan the plugins in the potentially active Profiles in this project and return a fully resolved
     * list. Note that while updating the {@link Plugin} reference returned will be reflected in the Model as it is the
     * same object, if you wish to remove or add items to the Model then you must use {@link #getModel()}
     *
     * @param session MavenSessionHandler, used by {@link PropertyResolver}
    ...
    /**
     * This method will scan the plugins in the pluginManagement section in the potentially active Profiles in this
     * project and return a fully resolved list. Note that while updating the {@link Plugin} reference returned will be
     * reflected in the Model as it is the same object, if you wish to remove or add items to the Model then you must
     * use {@link #getModel()}
     *
     * @param session MavenSessionHandler, used by {@link PropertyResolver}
    ...
    /**
     * This method will scan the dependencies in this project and return a fully resolved list. Note that this will only
     * return full dependencies not managed i.e. those with a group, artifact and version. Note that while updating the
     * {@link Dependency} reference returned will be reflected in the Model as it is the same object, if you wish to
    ...
    /**
     * This method will scan the dependencies in this project and return a fully resolved list. Note that this will
     * return all dependencies including managed i.e. those with a group, artifact and potentially empty version. Note
     * that while updating the {@link Dependency} reference returned will be reflected in the Model as it is the same
     * object, if you wish to remove or add items to the Model then you must use {@link #getModel()}
     *
     * @param session MavenSessionHandler, used by {@link PropertyResolver}
     * @return a list of fully resolved {@link ArtifactRef} to the original {@link Dependency}
     * @throws ManipulationException if an error occurs
    ...
    /**
     * This method will scan the dependencies for *every* plugin in this project and return a list of maps of fully
     * resolved version to dependency. Note that this will scan for plugins included managed plugins, build plugins and
     * those in profiles. It utilises a list to return as each plugin could have the same dependencies which would
     * conflict. Note that while updating the {@link Dependency} reference returned will be reflected in the Model as it
     * is the same object, if you wish to remove or add items to the Model then you must use {@link #getModel()}
     *
     * @param session MavenSessionHandler, used by {@link PropertyResolver}
    ...
    /**
     * This method will scan the dependencies in the dependencyManagement section of this project and return a fully
     * resolved list. Note that while updating the {@link Dependency} reference returned will be reflected in the Model
    ...
    private void resolveDeps(MavenSessionHandler session, List<Dependency> deps, boolean includeManagedDependencies,
            Map<ArtifactRef, Dependency> resolvedDependencies) throws ManipulationException {
        ...
        while (iterator.hasPrevious()) {
            ...
            if (isNotEmpty(g) && isNotEmpty(a) && isNotEmpty(v)) {
                SimpleScopedArtifactRef sar = new SimpleScopedArtifactRef(g, a, v, d.getType(), d.getClassifier(),
                        d.getScope());
                ...
                // the indexing as we don't have duplicate entries. Given they are exact matches, remove older
                // duplicate.
    ...
    private void resolvePlugins(MavenSessionHandler session, List<Plugin> plugins, PluginResolver includeManagedPlugins,
            Map<ProjectVersionRef, Plugin> resolvedPlugins) throws ManipulationException {
        ...
        while (iterator.hasPrevious()) {
            ...
            // Its possible the internal plugin list is either abbreviated or empty. Attempt to fill in default values
            // for
            // comparison purposes.
            if (isEmpty(g)) {
                ...
                if (resolvedPlugins.containsKey(spv)) {
                    ...
                    if (old != null) {
                        logger.error("Internal project plugin resolution failure ; replaced {} in store by {}.", old,
                                spv);
                        throw new ManipulationException(
                                "Internal project plugin resolution failure ; replaced {} in store by {}.", old, spv);
                    }
                }
            }
        }
    ...
    public void setInheritanceRoot() {
