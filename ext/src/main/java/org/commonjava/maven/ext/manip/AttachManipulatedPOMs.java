...
@Mojo(name = ManipulationExtensionBridge.GOAL_ATTACH_MODIFIED_POMS, instantiationStrategy = InstantiationStrategy.SINGLETON, threadSafe = true)
public class AttachManipulatedPOMs extends AbstractMojo {

    final private MavenSession mavenSession;

    final private ManipulationExtensionBridge mavenBridge;
    
    @Parameter(property = "dummy", required = false, defaultValue = "")
    private Boolean dummy;

    @Inject
    AttachManipulatedPOMs(MavenSession mavenSession, ManipulationExtensionBridge mavenBridge, PomIO pomIO) {
    ...
    public void execute() throws MojoExecutionException {
        ...
        if (optReport.isEmpty()) {
            getLog().warn(ManipulationExtensionBridge.GOAL_ATTACH_MODIFIED_POMS
        ...
        try {
            ...
            throw new MojoExecutionException(
                    "Unable to execute goal: " + ManipulationExtensionBridge.GOAL_ATTACH_MODIFIED_POMS, ex);
        }
    }
    ...
    void attachModifiedPomFilesToTheProject(List<MavenProject> projects, Set<GAV> gavs, String version,
        ...
        for (MavenProject project : projects) {
            ...
            writeModelPom(project.getOriginalModel(), newPom);
    ...
    File createPomDumpFile() throws IOException {
        File tmp = File.createTempFile("pom", ".manipulation-ext.xml");
        tmp.deleteOnExit();
        return tmp;
    }
